...
| Assets | `client.Assets` | Pack `url` + `hash` from published tip; **Filter Packs For Download** (quality × OS × language); cache under `Saved/FastGame/packs` (UE) / `persistentDataPath/fastgame/packs` (Unity). **Publish tip** required — draft `/asset-packs` APIs are panel-only. |
...
| Me | GET / PATCH | `/base/login/me` (Bearer) → user public profile (no password), including nullable `residence_country_code` / `residence_subdivision_code`; PATCH supports self-service full-name and residence updates |
| Me update | PATCH | `/base/login/me` (Bearer, JSON: optional `full_name`, nullable `residence_country_code` / `residence_subdivision_code`) — self profile only; does not change the login id |
...
| Signup | POST | `/base/users/signup` (JSON: `email?`, `phone?`, `password`, `full_name?`, nullable `residence_country_code?` / `residence_subdivision_code?`, optional `game_code` / `code`) — **new** users only (**400** if already exists). With `game_code` + channel verify enabled, requires prior signup OTP (`signup_verified` or one-shot `code`). SDKs: empty Email+Phone → ENTER store; require password confirm locally (not sent), then auto-login. Existing Register node is unchanged; **Register With Residence** adds residence inputs. |
...
Prefer seat over public `Catalog.GetGameServer` / `GET /utils/game-server/` for online join. Fast Game ships a thin `MintSeat` / `JoinMap` HTTP helper; full Colyseus wrap is **not** required inside this plugin.

## Entity components (LEVEL prefabs)

Slim **Blueprint-spawnable components** bind locale-free catalog **NAME**s on prefabs. Config/ownership comes from Fast-Game tip + Progress/Shop; meshes, portraits, and UI use **local project assets**.
...
| Module | Role |
|--------|------|
| **GameplayDirector** | `Boot Gameplay` → GetMapConfig; apply `camera_profile` / `input_profile_id`; façade SetAnimator / Activate Ability; boots **Flow Runtime** |
| **Flow Runtime** | Walks `flow_runtime` (V3): play_dialogue, load_level, branch, unlock_achievements; Notify Trigger Enter; Fellow gate |
| **Character Controller** | Move / jump N / crouch / sprint; tip locomotion + Speed animator bind (V2). Legacy: Movement Runtime |
| **Camera Controller** | `camera_profile` tps \| fps \| top_down \| side (aliases: tps_follow). Legacy: Camera Runtime |
| **ParamRuntime** | SetAnimator / SetMaterial (declared param NAMEs) |
| **AbilityRuntime** | Activate Ability → tip `on_activate` / `on_deactivate`; Load Abilities From Tip Json |
| **LootRuntime** | Get Loot Table + Open Loot (server roll; forged grants rejected) |

Character tip (Get Character) lifts `movement_profile`, `camera_profile`, `cameras[]`, jump/walk fields from `stats` plus ability binds.
...
- **Traveled** — solo/offline: target map `engine_scene` opened via `OpenLevel`.
- **Matchmaking** — online mode: seat minted (`FFastGameBPSeatMint`); join sibling Colyseus with `seat_token`, then open level.
...
Quest listener: wire `Notify Quest Complete|Failed|Not Started Yet` from the Flow driver, or bind `OnQuestComplete` / `OnQuestFailed` / `OnQuestNotStartedYet`.
